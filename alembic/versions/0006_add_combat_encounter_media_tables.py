...
def upgrade():
    ...
    if "combataction" not in existing_tables:
        ...
        try:
            op.create_foreign_key(
                "fk_combataction_playerclass", "combataction", "playerclass", ["requires_class"], ["id"]
            )
        except Exception:
            pass
        try:
            op.create_foreign_key("fk_combataction_playerrace", "combataction", "playerrace", ["requires_race"], ["id"])
        except Exception:
            pass
    ...
    if "encounter" not in existing_tables:
        ...
        # Add foreign keys
        try:
            op.create_foreign_key("fk_encounter_tile", "encounter", "tile", ["tile_id"], ["id"], ondelete="CASCADE")
        except Exception:
            pass
        try:
            op.create_foreign_key("fk_encounter_user", "encounter", "user", ["user_id"], ["id"])
        except Exception:
            pass
        try:
            op.create_foreign_key(
                "fk_encounter_combataction", "encounter", "combataction", ["combat_action_id"], ["id"]
            )
        except Exception:
            pass
    ...
    if "tilemedia" not in existing_tables:
        ...
        try:
            op.create_foreign_key(
                "fk_tilemedia_tiletypeoption", "tilemedia", "tiletypeoption", ["tile_type_id"], ["id"]
            )
        except Exception:
            pass
        try:
            op.create_foreign_key("fk_tilemedia_tile", "tilemedia", "tile", ["tile_id"], ["id"], ondelete="CASCADE")
        except Exception:
            pass
